    ...
    def train(self, X, Y, max_iter=2):
        for _ in range(max_iter):
            for i in range(X.shape[0]):
                ### TODO: Update weights
                class_hat = self.pred(X[i])
                if Y[i] == class_hat:
                    continue;

                self.weights[class_hat] -= X[i]
...
if __name__ == '__main__':
    ...
    #3.2a
    test_error = []
    lst = [10,100,1000,10000,50000,80000]
    for num_train_samples in lst:
    ...
    plt.plot(lst,test_error);
    plt.xscale("log")
    plt.xlabel('Training samples in log scale')
