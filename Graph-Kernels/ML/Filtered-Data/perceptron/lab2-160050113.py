...
class Perceptron():
    ...
    def pred(self, x):
        ...
        mul = np.matmul(self.weights,x)
        allhigh = np.argwhere(mul == np.amax(mul))
        allhigh_list = list(allhigh.reshape(allhigh.shape[0]))
        randomhigh = np.random.choice(allhigh_list)
        return randomhigh
        ### END TODO

    def train(self, X, Y, max_iter=12):
        for _ in range(max_iter):
            for i in range(X.shape[0]):
                ### TODO: Update weights
                predicted_class = self.pred(X[i,:])
                if predicted_class != Y[i]:
                    self.weights[predicted_class,:] -= X[i,:]
