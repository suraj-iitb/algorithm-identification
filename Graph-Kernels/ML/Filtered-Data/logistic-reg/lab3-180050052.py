...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        score = score.astype(np.float128)
        score = np.exp(score)       
        #score = np.array([math.exp(x) for x in score])
        denom = np.repeat(np.sum(score, axis = 1).reshape(-1,1), self.C, axis=1)
        softmax = np.divide(score, denom)
        return softmax
        # END TODO
    ...
    def train(self, X, Y, lr=5, max_iter=500):
        ...
        for i in range(max_iter):
            ...
            cost = Y - self.softmax(X)
            gradient = np.dot(np.transpose(X), cost)/(X.shape[0])
