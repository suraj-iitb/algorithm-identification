...
template<typename T, typename S, T (*op)(T, S)>
struct Dijkstra {
  int n;
  WeightedGraph<S> es;
  vector<T> dist;
  vector<int> prev;
  Dijkstra(int _n, WeightedGraph<S> _es) {
    init(_n, _es);
  }
  void init(int _n, WeightedGraph<S> _es) {
    n = _n; es = _es;
  }
  void reset() {
    dist.assign(n, LINF);
  ...
  void run(int s) {
    ...
    while (!pq.empty()) {
      ...
      for (int i=0; i<(int)es[v].size(); i++) {
        edge<S> e = es[v][i];
        T to_dist = op(dist[v], e.cost);
        if (dist[e.to] > to_dist) {
          dist[e.to] = to_dist;
...
void solve()
{
  ...
  Dijkstra<ll, ll, op> dij(v, g);
  dij.run(r);
  rep(i, v) {
    if (dij.dist[i] == LINF) cout << "INF" << "\n";
    else cout << dij.dist[i] << "\n";
  }
