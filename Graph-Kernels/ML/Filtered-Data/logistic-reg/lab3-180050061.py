...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        pr=np.matmul(X,self.weights)
        pr=np.exp(pr)
        su=np.sum(pr,axis=1)
        return np.transpose(np.transpose(pr)/su)
    ...
    def train(self, X, Y, lr=0.1, max_iter=500):
        ...
        for i in range(max_iter):
            ...
            xw=np.exp(xw)
            cs=np.sum(xw,axis=1)
            xw=np.transpose(np.transpose(xw)/cs)
            grad=np.matmul(np.transpose(X),(Y-xw))/np.shape(X)[0]
