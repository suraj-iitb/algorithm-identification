...
template<typename T>
void countingSort(vector<T> &seq) {
    int n = seq.size();
    int maxValue = *max_element(seq.begin(), seq.end());
    vector<int> index(maxValue+1,0);
    vector<T> cp(seq);
    for (int i = 0;i < n;i++) index[seq[i]]++;
    for (int i = 0;i < maxValue;i++) index[i+1] += index[i];
    for (int i = n-1;i >= 0;i--) {
        seq[index[cp[i]]-1] = cp[i];
        index[cp[i]]--;
    }
}
