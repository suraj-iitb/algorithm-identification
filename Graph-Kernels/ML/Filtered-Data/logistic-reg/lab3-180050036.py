...
class LogisticRegression:
    ...
    def predict(self, X):
        ...
        soft = self.softmax(X)
        n = X.shape[0]
        maxi = np.argmax(soft, axis = 1)
        #maxi = maxi.reshape([n,1])
        return maxi
        # END TODO

    def train(self, X, Y, lr=0.1, max_iter=150):
        ...
        for i in range(max_iter):
            ...
            n = X.shape[0]
            first = Y
            second = self.softmax(X)
            final = first-second
            final = (-1/n)*final
            final = (X.T)@final
            self.weights = self.weights - lr*(final)
            # END TODO

            # TODO: Stop the algorithm if the norm of the gradient falls below 1e-4
            if np.linalg.norm(final) < 1e-4:
                break
            # End TODO
