...
// ダイクストラ法 計算量 O( log(頂点数N) * 辺の数E)
// s...始点 N...頂点の数 D...最短距離を記録するvector G...重み付きグラフ
template<typename T>
void dijkstra(int s, int N, vector<T>& D, WG<T> &G ){ 
  using P = pair<T,int>; // pair(最短距離,頂点の番号)
  ...
  while(!Q.empty()){
    ...
    for(auto &e : G[v]) // G 重み付きグラフ
      if( amin(D[e], D[v] + e.cost) ) Q.emplace(D[e], e);
  }
}
