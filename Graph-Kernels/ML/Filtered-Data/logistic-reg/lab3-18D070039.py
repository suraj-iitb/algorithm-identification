...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        f=one.shape[0]
        g=one.shape[1]
        ss=np.tile(one.sum(axis=1).reshape((f,1)),(1,g))
        smax=np.divide(one,ss)
        # print(ss)
        return smax
        # END TODO
    ...
    def train(self, X, Y, lr=0.1, max_iter=500):
        ...
        for i in range(max_iter):
            ...
            smax=self.softmax(X)
            #smax=self.predict(X)
            maxx=np.zeros_like(smax)
            maxx[np.arange(len(smax)),smax.argmax(1)]=1
            # print(maxx)
            ss=smax-Y
            ss=np.matmul(np.transpose(X),ss)
            self.weights= W-(lr)*ss/N
            # print(i)
            #break
            # END TODO
