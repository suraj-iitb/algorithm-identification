...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        phi_W = X @ self.weights
        e_phi_W = np.exp(phi_W)

        softmax_ret = e_phi_W / e_phi_W.sum(axis=1)[:,None]
    ...
    def train(self, X, Y, lr=0.8, max_iter=500):
        ...
        for i in range(max_iter):
            ...
            soft = self.softmax(X)
            # y_soft = np.multiply(Y, soft)
            y_minus_soft = Y - soft

            delta_E = (-1) * (1/X.shape[0]) * (X.T @ y_minus_soft)

            W_new = self.weights - (lr * delta_E)

            # END TODO

            # TODO: Stop the algorithm if the norm of the gradient falls below 1e-4

            diff = np.linalg.norm(delta_E)
            self.weights = W_new
            # print(diff)
            if diff < 1e-4:
                break
