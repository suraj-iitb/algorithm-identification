...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        ret = X @ self.weights
        softmax = np.exp(ret)/np.sum(np.exp(ret), axis=1).reshape(X.shape[0], 1)
        return softmax
        # END TODO
    ...
    def predict(self, X):
        ...
        pred = np.argmax(sftmax, axis=1)
        return pred
        # END TODO

    def train(self, X, Y, lr=3, max_iter=100):
