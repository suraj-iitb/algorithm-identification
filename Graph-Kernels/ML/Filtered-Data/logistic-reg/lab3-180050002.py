...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        intr = np.exp(X@self.weights)
        return intr/np.sum(intr,axis = 1).reshape(-1,1)
        # END TODO
    ...
    def train(self, X, Y, lr=0.3, max_iter=100):
        ...
        for i in range(max_iter):
            ...
            self.weights -= lr*(X.T@(self.softmax(X)-Y))/X.shape[0]
            # END TODO

            # TODO: Stop the algorithm if the norm of the gradient falls below 1e-4
            if np.linalg.norm(self.weights)<0.0001:
                return
            # End TODO
