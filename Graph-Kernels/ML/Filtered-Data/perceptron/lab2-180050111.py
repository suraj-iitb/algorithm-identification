...
class Perceptron():
    ...
    def pred(self, x):
        ...
        pred_class = np.argmax(np.matmul(self.weights, x))
        return pred_class
        ### END TODO
    ...
    def train(self, X, Y, max_iter=10):
        for _ in range(max_iter):
            for i in range(X.shape[0]):
                ### TODO: Update weights
                x = X[i]
                pred_class = self.pred(x)
                self.weights[pred_class] -= x
                self.weights[Y[i]] += x
                ### END TODO
...
if __name__ == '__main__':
    ...
    ## test set = D1, plot for perceptron with increasing train data
    # num_train_samples_lst = [10, 100, 1000, 10000, 50000, 80000]
    # test_err = []
    # for num_train_samples in num_train_samples_lst:
    ...
    #     perceptron = Perceptron(C, D)
    #     perceptron.train(X_train, Y_train)
    #     err = 1 - perceptron.eval(X_test, Y_test)
    #     test_err.append(err)
    #     print(err)

    # plt.scatter(num_train_samples_lst, test_err)
    # plt.plot(num_train_samples_lst, test_err)
    # plt.xscale('log')
    # plt.xlabel('number of train samples')
    # plt.ylabel('Test Error')
    # plt.title('Test Error (on D1) with Perceptron vs number of Train Samples')
    # plt.show()
