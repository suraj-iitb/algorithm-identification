        ...
        for _ in range(max_iter):
            for i in range(X.shape[0]):
                ...
                if y1!=Y[i]:
                    ...
                    self.weights[y1]=self.weights[y1]-X[i]
                ### END TODO
...
if __name__ == '__main__':
    ...
    #     print(num[i])
    #     X_train,Y_train,X_test,Y_test=get_data('D1',num[i])
    #     C = max(np.max(Y_train), np.max(Y_test))+1
    #     D = X_train.shape[1]
    #     perceptron = Perceptron(C, D)
    #     perceptron.train(X_train, Y_train)
    ...
    # ax.xaxis.set_ticklabels(num)
    # plt.plot(range(0,6),test)
    # plt.xlabel('Number of Train samples')
    # plt.ylabel('Test error')
    # plt.title('Perceptron: Test error vs Train samples')
