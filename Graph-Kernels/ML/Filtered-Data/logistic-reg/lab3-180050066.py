...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        sm = np.dot(X,self.weights)
        smh = np.exp(sm)
        sm = smh.T/np.sum(smh, axis=1)
        sm = sm.T
        # END TODO
        return sm
    ...
    def train(self, X, Y, lr=0.1, max_iter=500):
        ...
        for i in range(max_iter):
            ...
            a = self.softmax(X)
            b = self.weights
            self.weights = self.weights - lr*(np.dot(X.T,a-Y)/n)
            # END TODO

            # TODO: Stop the algorithm if the norm of the gradient falls below 1e-4
            if np.linalg.norm(self.weights-b)<0.0001:
                break
            # End TODO
