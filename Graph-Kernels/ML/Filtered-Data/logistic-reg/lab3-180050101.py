...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        contribution = np.exp((X @ self.weights))
        normalize = np.sum(contribution, axis=1)
        return contribution / normalize.reshape((X.shape[0], 1))
        # END TODO
    ...
    def train(self, X, Y, lr=3, max_iter=500):
        ...
        for i in range(max_iter):
            ...
            gradient = (X.T @ (self.softmax(X) - Y)) / X.shape[0]
