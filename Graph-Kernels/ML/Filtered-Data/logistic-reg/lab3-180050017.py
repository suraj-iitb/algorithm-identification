...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        num = np.exp(X@self.weights)
        den = np.sum(num,axis=1).reshape(X.shape[0],1)
        #print(den.shape)
        return num/den
        # END TODO
    ...
    def train(self, X, Y, lr=0.1, max_iter=500):
        ...
        for i in range(max_iter):
            ...
            soft = self.softmax(X)
            soft = X.T@soft/X.shape[0]
            W_prev = self.weights.copy()
            self.weights = self.weights + lr*(fir-soft)
            # END TODO

            # TODO: Stop the algorithm if the norm of the gradient falls below 1e-4
            if(np.linalg.norm(fir-soft)<0.0001):
                break;
            # End TODO
