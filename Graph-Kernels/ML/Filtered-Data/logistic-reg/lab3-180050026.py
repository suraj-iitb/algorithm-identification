...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        inter = np.exp(inter)
        inter2 = np.sum(inter , axis = 1)
        inter2 = inter2.reshape((X.shape[0] , 1))
    ...
    def train(self, X, Y, lr=0.5, max_iter=125):
        ...
        for i in range(max_iter):
            ...
            self.weights= self.weights - (lr/X.shape[0])* (np.dot(X.T , (self.softmax(X)-Y)))
            # END TODO

            # TODO: Stop the algorithm if the norm of the gradient falls below 1e-4
            norm_val =np.linalg.norm((1/X.shape[0])* (np.dot(X.T , (self.softmax(X)-Y))))
            if(norm_val < 1e-4):
                break
            # End TODO
