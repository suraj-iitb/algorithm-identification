...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        Z_exp = np.exp(Z)
        Z_row = np.sum(Z_exp,axis=1,keepdims=True)
        Z_denom = 1/Z_row

        soft = np.multiply(Z_exp,Z_denom)
    ...
    def train(self, X, Y, lr=0.1, max_iter=500):
        ...
        for i in range(max_iter):
            ...
            #Y_hat = np.multiply(Y,Y_hat)
            Z_dash = Y - Y_hat
            Z_dash = -1*Z_dash/X.shape[0]
            grad = np.transpose(X).dot(Z_dash)
