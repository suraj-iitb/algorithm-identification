...
def mse(X, Y, W):
	...
	## TODO
	mse = np.sum(np.square(np.matmul(X, W) - Y))/(2 * Y.shape[0])
...
def ridge_regression(X_train, Y_train, X_test, Y_test, reg, lr=0.001, max_iter=500):
	...
	for i in range(max_iter):
		...
		## TODO: Compute train and test MSE
		#regExtra = reg * np.matmul(np.transpose(W), W)
		train_mse = mse(X_train, Y_train, W)
		test_mse = mse(X_test, Y_test, W)
		## END TODO
		...
		## TODO: Update w and b using a single step of gradient descent
		W = W - lr*(np.matmul(np.transpose(X_train),(np.matmul(X_train, W) - Y_train)) / X_train.shape[0] + 2 * reg * W)
		## END TODO
