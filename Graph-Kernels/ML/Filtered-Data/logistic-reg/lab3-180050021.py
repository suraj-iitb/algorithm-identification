...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        Z = np.exp(np.matmul(X,self.weights))
        S = np.sum(Z,axis=1)
    ...
    def predict(self, X):
        ...
        sm = np.argmax(self.softmax(X),axis=1)
        return sm
        # END TODO

    def train(self, X, Y, lr=10, max_iter=20):
        ...
        for i in range(max_iter):
            ...
            dw = (1/X.shape[0])*np.matmul(np.transpose(X),self.softmax(X)-Y)
