...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        n = X.shape[0]
        WX = np.exp(np.dot(X,self.weights))
        S = np.reciprocal(np.sum(WX,axis=1)).reshape((n,1))
        SM = np.multiply(WX,S)
        return SM
        # END TODO
    ...
    def predict(self, X):
        ...
        n = X.shape[0]
        SM = self.softmax(X)
        P = np.apply_along_axis(np.argmax,1,SM)
        return P
        # END TODO
    ...
    def train(self, X, Y, lr=0.1, max_iter=500):
        ...
        for i in range(max_iter):
            ...
            grad = (-1/X.shape[0])*np.dot(np.transpose(X),(Y-self.softmax(X)))
