...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        L = np.exp(np.matmul(X,self.weights))
        return np.divide(L.T,np.sum(L,axis = 1)).T
        # END TODO
    ...
    def train(self, X, Y, lr=0.1, max_iter=500):
        ...
        for i in range(max_iter):
            ...
            self.weights = self.weights - (lr/X.shape[0])*(np.matmul(X.T,self.softmax(X)-Y))
            # END TODO
            
            # TODO: Stop the algorithm if the norm of the gradient falls below 1e-4
            if (1/X.shape[0])*np.linalg.norm(np.matmul(X.T,self.softmax(X)-Y)) < 0.0001:
                break
            # End TODO
