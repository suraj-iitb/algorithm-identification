...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        #weights are D x K, X is N x D, softmax is N x K, X.T is D x N, (X.T @ (softmax - Y)) is D x K
        
        nums = np.exp(X @ self.weights)
        
        nums /= np.sum(nums, axis=1).reshape(-1, 1)
        return nums
        # END TODO
    ...
    def train(self, X, Y, lr=0.1, max_iter=500):
        ...
        for i in range(max_iter):
            ...
            nabla = (lr/X.shape[0])*(X.T @ (self.softmax(X) - Y))
            self.weights = self.weights - nabla
            # END TODO

            # TODO: Stop the algorithm if the norm of the gradient falls below 1e-4
            if np.linalg.norm(nabla) < 1e-4:
                break
            # End TODO
