...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        mat = np.exp(np.dot(X, self.weights))
        sum_row = np.sum(mat, axis = 1)
        inv_sum_row = 1/sum_row.reshape([sum_row.shape[0], 1])
        inv_sum_row = inv_sum_row*mat
        return inv_sum_row
        # END TODO
    ...
    def predict(self, X):
        ...
        ans = np.argmax(soft_max, axis = 1)
        return ans
        # END TODO

    def train(self, X, Y, lr=0.3, max_iter=120):
