...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        temp = X.dot(self.weights)

        # func = lambda t: math.exp(t)
        # applyfunc = np.vectorize(func)
        # temp = applyfunc(temp)
        ...
        ###sm = np.diag(row_sum).dot(temp)

        sm = temp/temp.sum(axis=1, keepdims=True)
        # print(sm.shape)

        # sm = temp / row_sum[:,None]
    ...
    def predict(self, X):
        ...
        answer = self.softmax(X).argmax(axis=1)
        return answer
    ...
    def train(self, X, Y, lr=0.1, max_iter=500):
        ...
        for i in range(max_iter):
            ...
            # b[np.arange(N),Y.reshape(Y.shape[0])] = 1
            ...
            gradient = (   X.transpose().dot(temp) )

            W = self.weights - (lr*gradient)
            ...
            if (np.linalg.norm(gradient)<0.0001):
                self.weights = W
                break
            # print(np.linalg.norm(gradient/N))   
            self.weights = W
