...
def ordinary_least_squares(X_train, Y_train, X_test, Y_test, lr=0.001, max_iter=2000):
	...
	for i in range(max_iter):
		...
		W = W - lr*np.transpose(X_train).dot(X_train.dot(W)-Y_train)/X_train.shape[0]
		## END TODO
...
def ridge_regression(X_train, Y_train, X_test, Y_test, reg, lr=0.001, max_iter=5000):
	...
	for i in range(max_iter):
		...
		## TODO: Update w and b using a single step of gradient descent
		W = W - lr*(np.transpose(X_train).dot(X_train.dot(W)-Y_train)/X_train.shape[0])-lr*2*reg*W
		print(train_mse)
		## END TODO
