...
class Perceptron():
    ...
    def pred(self, x):
        ...
        x.shape=(x.size,1)
        x1=np.dot(self.weights, x)
        y=np.argmax(x1)
        return y
        # err=[0.7367, 0.47824999999999995, 0.21975, 0.22124999999999995, 0.2258, 0.20094999999999996]
        ### END TODO
    ...
    def train(self, X, Y, max_iter=2):
        for _ in range(max_iter):
            for i in range(X.shape[0]):
                ### TODO: Update weights
                pred_cat = self.pred(X[i])
                
                if Y[i] != pred_cat:
                    self.weights[Y[i]] = self.weights[Y[i]] + X[i]
                    self.weights[pred_cat] = self.weights[pred_cat] - X[i]
                ### END TODO
