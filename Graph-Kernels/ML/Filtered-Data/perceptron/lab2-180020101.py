...
class Perceptron():
    ...
    def pred(self, x):
        ...
        predictor=np.matmul(self.weights,x)
        return np.argmax(predictor)
        ### END TODO
        ...
        for _ in range(max_iter):
            for i in range(X.shape[0]):
                ...
                if prediction!=Y[i]:
                    ...
                    # print("******")
                    self.weights[Y[i]]=np.add(X[i],self.weights[Y[i]])
                    self.weights[prediction]=np.subtract(self.weights[prediction],X[i])
                # if no match add in actual  and subtract in predicted
                ### END TODO
