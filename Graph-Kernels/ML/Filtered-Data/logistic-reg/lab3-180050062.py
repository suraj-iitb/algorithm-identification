...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        unnorm_sm = np.exp(np.matmul(X, self.weights))
        sm = unnorm_sm/np.sum(unnorm_sm, axis=1, keepdims=True)
        return sm
        # END TODO
    ...
    def train(self, X, Y, lr=0.1, max_iter=1500):
        ...
        for i in range(max_iter):
            ...
            grad = lr*np.matmul(X.T, (self.softmax(X) - Y))/X.shape[0]
            self.weights -= grad
            # END TODO
