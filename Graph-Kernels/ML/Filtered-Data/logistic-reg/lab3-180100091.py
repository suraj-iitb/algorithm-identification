...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        return np.exp(np.dot(X,self.weights))/np.array([np.sum(np.exp(np.dot(X,self.weights)),axis=1)]).transpose()#1/(1+np.exp(-np.dot(X,self.weights)))
        # END TODO
    ...
    def train(self, X, Y, lr=0.1, max_iter=1000):
        ...
        for i in range(max_iter):
            ...
            n = X.shape[0]
            sigma_w = self.softmax(X)#1/(1+np.exp(-np.dot(X,self.weights)))
            grad = np.dot(X.transpose(),sigma_w-Y)/n
