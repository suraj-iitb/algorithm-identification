...
# for algo in data:
print(algo)
for file in os.listdir('../MOSS/Data/Path/'+algo+'/C'):
    print(file)
    if file.endswith('.c'):
        os.makedirs(inp1)
        os.system('cp ../MOSS/Data/Path/'+algo+'/C/' + file + ' ' + inp1)
            ...
            with open(out1 + '/'+dot_file, 'r') as f:
                ...
                if len(temp) == 3:
                    ...
                    with open(out1 + '/out.dot', 'w') as out:
                        out.write(''.join(temp))
                    os.system('graph-easy ' + out1 +'/out.dot --graphml --out CFG/Path/'+ algo + '/' + file.split('.')[0] + '.graphml')
