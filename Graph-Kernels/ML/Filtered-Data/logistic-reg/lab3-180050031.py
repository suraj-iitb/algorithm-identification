...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        c= np.exp(np.matmul(X,self.weights))
        d=np.reciprocal(np.sum(c,axis=1)).reshape(c.shape[0],1)
        soft = np.multiply(c,d)
        return soft
        # END TODO
    ...
    def train(self, X, Y, lr=0.3, max_iter=500):
        ...
        for i in range(max_iter):
            ...
            grad = np.matmul(np.transpose(X),self.softmax(X)-Y)/X.shape[0]
