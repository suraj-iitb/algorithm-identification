...
int main(){
    ...
    vector<vector<long long int>> edge(v, vector<long long int>(v, 1e18));
    REP(i, v) edge[i][i] = 0;
    REP(i, e){
        LL s, t, d; scanf("%lld%lld%lld", &s, &t, &d);
    ...
    REP(l, 2){
        REP(k, v){
            REP(i, v){
                REP(j, v){
                    long long int cost = edge[i][k] + edge[k][j];
                    if (edge[i][j] > cost){
                        if (l == 1) jcy = false;
                        else edge[i][j] = cost;
                    }
    ...
    if (jcy){
        REP(i, v) REP(j, v) { if (edge[i][j] > 1e10) OT("INF"); else OT(edge[i][j]);
        if (j != v-1) OT(" "); else OT("\n");
    }
    }
    else {
