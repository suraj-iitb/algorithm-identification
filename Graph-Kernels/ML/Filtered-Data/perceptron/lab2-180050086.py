...
class Perceptron():
    ...
    def pred(self, x):
        ...
        return np.argmax(self.weights @ x.reshape(x.shape[0],1))
        ### END TODO
        ...
        for _ in range(max_iter):
            for i in range(X.shape[0]):
                ...
                if ypred == Y[i]:
                    ...
                    self.weights[ypred, :] = self.weights[ypred, :] - X[i]
                ### END TODO
...
if __name__ == '__main__':
    ...
    nvec = [10,100,1000,10000,50000,80000]
    test_errors = []
    for num_train_samples in nvec:
    ...
    plt.plot(nvec, test_errors)
    plt.xscale("log")
    plt.xlabel('Num samples (log scale)')
