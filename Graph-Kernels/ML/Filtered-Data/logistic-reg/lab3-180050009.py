...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        # N = X.shape[0]
        # sof = np.zeros((N,self.C))
        sof = np.dot(X,self.weights)
        # for i in range(X.shape[0]):
        # 	norm = 0
        # 	for j in range(self.C):
        # 		z = np.matmul(X[i],self.weights.T[j])
        # 		sof[i][j] = np.exp(z)
        # 		norm = norm + sof[i][j]
        # 	sof[i]=sof[i]/norm
        sof1 = (np.exp(sof).T / np.sum(np.exp(sof),axis=1)).T
        return sof1
        # END TODO
    ...
    def train(self, X, Y, lr=0.1, max_iter=1000):
        ...
        for i in range(max_iter):
            ...
            if(np.linalg.norm(grad)<np.exp(-4)):
                break
            # END TODO
