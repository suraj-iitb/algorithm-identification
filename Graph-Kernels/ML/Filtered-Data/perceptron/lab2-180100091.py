    ...
    def train(self, X, Y, max_iter=2):
        for _ in range(max_iter):
            for i in range(X.shape[0]):
                ### TODO: Update weights
                predict = self.pred(X[i])
                if(predict!=Y[i]):
                    self.weights[predict]-=X[i]
...
if __name__ == '__main__':
    ...
    # x = [10,100,1000,10000,50000,80000]
    # y = [1-0.2633,1-0.52175,1-0.78025,1-0.77875,1-0.7742,1-0.79905]
    ...
    # ax.set_title('Test error plotted against num_samples') 
    # plt.show()
