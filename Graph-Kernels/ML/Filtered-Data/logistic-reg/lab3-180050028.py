...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        w_transpose_phi = np.dot(X,self.weights)
        e_power = np.exp(w_transpose_phi)
        sums  = np.sum(e_power,axis=1).reshape((e_power.shape[0],1))
        corresponding_sums = np.repeat(sums,e_power.shape[1],axis=1)
        return(e_power/corresponding_sums)
        # END TODO
    ...
    def train(self, X, Y, lr=0.1, max_iter=500):
        ...
        for i in range(max_iter):
            ...
            difference = Y - self.softmax(X)
            gradient = np.dot(np.transpose(X),difference)*(lr/X.shape[0])
            # END TODO
