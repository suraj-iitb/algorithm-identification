...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        N = X.shape[0]
        scores=np.matmul(X,self.weights)
        softmax = np.exp(scores)
        denom = np.sum(softmax,axis=1)
    ...
    def predict(self, X):
        ...
        return np.argmax(activation,axis=1).reshape(X.shape[0],1)
        # END TODO
    ...
    def train(self, X, Y, lr=0.1, max_iter=500):
        ...
        for i in range(max_iter):
            ...
            gradient = np.dot(X.T, Y-soft_max)/(Y.shape[0])
    ...
    def eval(self, X, Y):
        ...
        N=preds.shape[0]
        accuracy = ((preds == Y.reshape(N,1)).sum()) / len(preds)
        return accuracy
