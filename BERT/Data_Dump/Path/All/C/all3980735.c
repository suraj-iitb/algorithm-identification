...
void Warshall_Froyd(W_graph& g, vector<vector<long>>& res) {
	...
	for (int i = 0; i < V; i++) {
		for (int j = 0; j < V; j++) {
			for (int k = 0; k < V; k++) {
                if (max(dist[j][i], dist[i][k]) == INT_MAX) continue;
...
int main() {
	...
	Bellman_Ford(G, 0, bel, negcir);
	Warshall_Froyd(G, res);

	if (negcir) cout << "NEGATIVE CYCLE" << endl;
	else for (int i = 0; i < V; i++) {
		for (int j = 0; j < V; j++) { if (res[i][j] == INT_MAX) cout << "INF"; else cout << res[i][j]; if (j != V - 1) cout << " "; };
        cout << endl;
	}
}
