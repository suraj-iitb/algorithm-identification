...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        v = np.exp(X @ self.weights)
        return v / np.reshape(np.sum(v, axis = 1), (len(v), 1))
        # END TODO
    ...
    def train(self, X, Y, lr=0.1, max_iter=200):
        ...
        for i in range(max_iter):
            ...
            n = len(X)
            W = self.weights + (lr / n) * (X.T @ (Y - self.softmax(X)))
            # END TODO

            # TODO: Stop the algorithm if the norm of the gradient falls below 1e-4
            if np.sum((W - self.weights) ** 2) <= (lr * 0.0001) ** 2:
                self.weights = W
                break
            self.weights = W
            # End TODO
