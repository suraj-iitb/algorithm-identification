...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        num = np.exp(np.dot(X,self.weights))
        #print(type(num))
        den = np.array([np.sum(num,axis=1)])
        #print(type(den))
    ...
    def train(self, X, Y, lr=0.1, max_iter=400):
        ...
        for i in range(max_iter):
            ...
            gradient = np.dot(np.transpose(X),self.softmax(X)-Y)/X.shape[0]
    ...
    def eval(self, X, Y):
        """
        X - numpy array of shape (N, D) corresponding to input data
