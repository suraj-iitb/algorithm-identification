...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        h = np.exp(X @ self.weights)
        h = h / np.sum(h,1).reshape(X.shape[0],1)
        return h
        # END TODO
    ...
    def predict(self, X):
        ...
        return np.argmax(X @ self.weights, 1)
        # END TODO
    ...
    def train(self, X, Y, lr=0.1, max_iter=500):
        ...
        for i in range(max_iter):
            ...
            h = self.softmax(X)
            dW = 1 / X.shape[0] * X.T @ (h - Y)
            # train_error = -1 / X.shape[0] * np.sum(Y * np.log(h + 1e-8))
