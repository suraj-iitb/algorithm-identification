...
class Perceptron():
    ...
    def pred(self, x):
        ...
        scores = np.matmul(self.weights, x.reshape(x.shape[0], 1))
    ...
    def train(self, X, Y, max_iter=2):
        for _ in range(max_iter):
            for i in range(X.shape[0]):
                ### TODO: Update weights
                class_pred = self.pred(X[i])
                if class_pred != Y[i]:
                    self.weights[Y[i]] += X[i] 
                    self.weights[class_pred] -= X[i]
                ### END TODO
...
def experiment():
    ...
    final_accuracies = []
    for num_training in num_train_samples:
        X_train, Y_train, X_test, Y_test = get_data('D1',num_training)
        ...
        final_accuracies.append(1.-acc)
    plt.figure()
    plt.title('Test set error v/s Number of Training Samples')
    plt.plot(num_train_samples, final_accuracies)
    plt.xscale('log')
