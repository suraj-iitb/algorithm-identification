...
class Perceptron():
    ...
    def pred(self, x):
        ...
        # return np.where(score == np.amax(score))[0][0]
    ...
    def train(self, X, Y, max_iter=2):
        for _ in range(max_iter):
            for i in range(X.shape[0]):
                ### TODO: Update weights
                pred_C = self.pred(X[i])
                if(pred_C != Y[i]):
                    self.weights[Y[i]] = self.weights[Y[i]] + X[i]
                    self.weights[pred_C] = self.weights[pred_C] - X[i]
                ### END TODO
...
if __name__ == '__main__':
    ...
    # For Plots in Q3 part a
    # iter = [10, 100, 1000, 10000, 50000, 80000]
    # plot_err = []
    # X_train0, Y_train0, X_test0, Y_test0 = get_data('D1',1000)
    # for i in iter:
    #     print("Iter : ",i)
    ...
    #     perceptron.train(X_train, Y_train)
    #     acc = perceptron.eval(X_test0, Y_test0)

    #     plot_err.append(1-acc)
