...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        m = np.matmul(X,self.weights)
        m = np.exp(m)
        row_sum_m = np.sum(m,axis=1)        
        N = np.shape(row_sum_m)[0]
        row_sum_m = row_sum_m.reshape((N,1))
        m = m/row_sum_m
        return m
        # END TODO
    ...
    def train(self, X, Y, lr=0.1, max_iter=500):
        ...
        for i in range(max_iter):
            ...
            N = np.shape(X)[0]
            gradient = np.matmul(np.transpose(X),(self.softmax(X) - Y))/N
