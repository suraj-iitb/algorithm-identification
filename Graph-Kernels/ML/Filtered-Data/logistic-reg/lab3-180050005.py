...
class LogisticRegression:
	...
	def softmax(self, X):
		...
		soft = np.exp(X @ self.weights)
		return soft/np.sum(soft, axis=1)[:,None]
		# END TODO

	def predict(self, X):
		"""
		:param X: numpy array of shape (N, D) corresponding to inpus data
		:return: numpy array of shape (N, ) corresponding to predicted labels
		"""
		# TODO: Return a (N, ) numpy array of predictions.
		return np.argmax(self.softmax(X), axis=1)
		# END TODO

	def train(self, X, Y, lr=0.2, max_iter=100):
		...
		for i in range(max_iter):
			...
			Y_hat = one_hot_encode(self.predict(X), np.arange(self.weights.shape[1]))
