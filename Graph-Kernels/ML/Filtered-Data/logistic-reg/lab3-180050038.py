...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        #its shape is m*c
        soft_arr=np.dot(X,self.weights)
        soft_arr=np.exp(soft_arr)
        for i in range(X.shape[0]):
            soft_arr[i]/=np.sum(soft_arr[i])
        return soft_arr
        # END TODO
    ...
    def predict(self, X):
        ...
        soft_arr=self.softmax(X)
        for i in range(X.shape[0]):
            ans[i]=np.argmax(soft_arr[i])
        return ans
        # END TODO

    def train(self, X, Y, lr=0.1, max_iter=170):
        ...
        for i in range(max_iter):
            ...
            grad=np.dot(X.T,self.softmax(X)-Y)/X.shape[0]
