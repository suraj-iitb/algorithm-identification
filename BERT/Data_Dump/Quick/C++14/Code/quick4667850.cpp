...
int partition(vector<tuple<char, int>>& A, int p, int r) {
    int x = get<1>(A[r]);
    int i = p - 1;
    FOR(j, p, r) {
        if (get<1>(A[j]) <= x) {
...
void quicksort(vector<tuple<char, int>>& A, int p, int r) {
    if (p < r) {
...
int main() {
    ...
    int n;
    INPUT(n);
    vector<tuple<char, int>> a(n);
    INPUT(a);
    vector<tuple<char, int>> b = a;
    quicksort(a, 0, n - 1);
    stable_sort(ALL(b), [](const auto& x, const auto& y) {
