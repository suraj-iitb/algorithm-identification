...
class LogisticRegression:
    ...
    def softmax(self, X):
        ...
        expPred  = np.exp(pred - np.max(pred,axis=1)[:,None])
        normPred = expPred / np.sum(expPred,axis=1)[:,None]
        return normPred
    ...
    def train(self, X, Y, lr=0.5, max_iter=200):
        ...
        for i in range(max_iter):
            ...
            dS = np.subtract(self.softmax(X),Y) / X.shape[0]
            dW = np.matmul(X.T,dS)
            W  = self.weights - lr*dW

            # END TODO

            # TODO: Stop the algorithm if the norm of the gradient falls below 1e-4
            if np.sqrt(np.mean(np.square(dW))) < 1e-4:
                break
            self.weights = W
