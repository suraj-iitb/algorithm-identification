...
class LogisticRegression:
    ...
    def predict(self, X):
        ...
        softmaxes = self.softmax(X)
        predictions= np.array([[np.argmax(x)] for x in softmaxes])
    ...
    def train(self, X, Y, lr=0.1, max_iter=500):
        ...
        for i in range(max_iter):
            ...
            self.weights = self.weights +(lr/N)*(np.matmul(np.transpose(X),Y-softmaxes))
            # END TODO

            # TODO: Stop the algorithm if the norm of the gradient falls below 1e-4
            if np.linalg.norm(self.weights-prev_weights)<1e-4:
                break
            # End TODO
