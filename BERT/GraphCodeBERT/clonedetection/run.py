...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='train'):
        ...
        url_to_code={}
        with open('/'.join(index_filename.split('/')[:-1])+'/data.txt') as f:
            lines = f.readlines()
            for i in range(0, len(lines)):
                line=lines[i].strip().split('<CODESPLIT>')
                if len(line) == 3:
                # js=json.loads(line)
                    url_to_code[str(i)]=line[2]
        print(len(url_to_code)) 
...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in range(args.epochs): 
        ...
        for step, batch in enumerate(bar):
            ...
            model.train()
            loss,logits = model(inputs_ids_1,position_idx_1,attn_mask_1, labels)
...
def evaluate(args, model, tokenizer, eval_when_training=False):
    ...
    # print(metrics.classification_report(y_trues, y_preds, digits=2))
