...
class Perceptron():
    ...
    def pred(self, x):
        ...
        # max1 = self.weights[0] @ x
        # max2 = max1[0][0]
        # index = 0
        # for i in range(self.weigts.shape[0]):
        #     ans = self.weights[i] @ x
        #     if(ans[0][0]>max2):
        #         max2 = ans[0][0]
        #         index = i
        # return index
        wrt = self.weights.dot(x)
        ans = np.argmax(wrt)
        return ans
    ...
    def train(self, X, Y, max_iter=2):
        for _ in range(max_iter):
            for i in range(X.shape[0]):
                ### TODO: Update weights decrease pred increase actual
                predy = self.pred(X[i])
                if(predy==Y[i]):
                    continue
                else:
                    self.weights[predy] = self.weights[predy] - X[i]
